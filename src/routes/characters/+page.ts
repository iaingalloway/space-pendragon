...
import { combatSkillKeys, nonCombatSkillKeys, type NullableSkillEntry, type SkillKey } from '$lib/skills.types';
import { type NullableParameterlessPassionEntry, type NullablePassionEntry } from '$lib/passions.types';
...
export const load: PageLoad = async () => {
  const traits = Object.fromEntries(
    traitKeys.map((key) => [
      key,
      { value: null, virtueChecked: false, viceChecked: false }
    ])
  ...
  const skills = Object.fromEntries(
    [...combatSkillKeys, ...nonCombatSkillKeys].map((key) => [
      key,
