<script lang="ts">
  ...
  import { labels, isParameterisedPassionEntry, getPassionKey } from '$lib/character.model';
					...
					<li class="border-outline flex items-center justify-between border-b py-1 last:border-0">
						...
						<span class="flex items-center gap-2">
							...
							<input
								...
								class="checkbox"
							/>
							<TraitShield value={character.traits[key].value == undefined ? null : 20 - character.traits[key].value} />
						</span>
					</li>
				{/each}
			</ul>
